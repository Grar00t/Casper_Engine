...
static int hbuf_app(HBuf *b, const char *data, size_t n) {
    if (!b || (!data && n != 0u)) return -1;
    if (n > HBUF_MAX - b->len - 1u) return -1;
    if (b->len + n + 1u > b->cap) {
        size_t need = b->len + n + 1u;
        size_t next = b->cap;
        while (next < need && next < HBUF_MAX) next *= 2u;
        if (next > HBUF_MAX) next = HBUF_MAX;
...
static HBuf *http_get(const char *host, const char *path) {
    ...
    HINTERNET session = WinHttpOpen(
        L"Casper/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    ...
    HINTERNET connection = WinHttpConnect(
        session, whost, INTERNET_DEFAULT_HTTPS_PORT, 0);
    if (!connection) {
    ...
    HINTERNET request = WinHttpOpenRequest(
        connection, L"GET", wpath, NULL, WINHTTP_NO_REFERER,
    ...
    WinHttpAddRequestHeaders(
        request, L"User-Agent: Casper/1.0\r\nAccept: text/html,application/xhtml+xml\r\n",
    ...
    if (ok) {
        ...
        while (WinHttpQueryDataAvailable(request, &available) && available > 0) {
            ...
            if (hbuf_app(out, chunk, (size_t)read) != 0) {
                ok = FALSE;
                break;
            }
        }
...
static void strip_tags(const char *html, char *out, size_t max) {
    ...
    for (const char *p = html; *p && n + 1u < max; ++p) {
        ...
        if (in_tag) continue;
        if (*p == '&') {
            if (strncmp(p, "&amp;", 5) == 0) { out[n++] = '&'; p += 4; }
            else if (strncmp(p, "&lt;", 4) == 0) { out[n++] = '<'; p += 3; }
            else if (strncmp(p, "&gt;", 4) == 0) { out[n++] = '>'; p += 3; }
            else if (strncmp(p, "&quot;", 6) == 0) { out[n++] = '"'; p += 5; }
            else if (strncmp(p, "&#39;", 5) == 0) { out[n++] = '\''; p += 4; }
            else if (strncmp(p, "&nbsp;", 6) == 0) { out[n++] = ' '; p += 5; }
            else out[n++] = '&';
            continue;
        }
...
static int parse_ddg(const char *html, RagResult *res, int max) {
    ...
    while (n < max && p && *p) {
        ...
        if (!gt) break;
        const char *title_end = strstr(gt + 1, "</a>");
        if (!title_end) title_end = strstr(gt + 1, "</");
        if (title_end) {
            char raw[RAG_TITLE_MAX * 2u];
            size_t len = (size_t)(title_end - (gt + 1));
            if (len >= sizeof(raw)) len = sizeof(raw) - 1u;
            memcpy(raw, gt + 1, len); raw[len] = '\0';
            strip_tags(raw, res[n].title, sizeof(res[n].title));
            p = title_end;
        }
        ...
        if (sn) {
            ...
            if (s0) {
                ...
                if (s1) {
                    ...
                    size_t len = (size_t)(s1 - s0);
                    if (len >= sizeof(raw)) len = sizeof(raw) - 1u;
                    memcpy(raw, s0, len); raw[len] = '\0';
                    strip_tags(raw, res[n].snippet, sizeof(res[n].snippet));
                    p = s1;
                }
            }
        }

        if (res[n].url[0] && (strncmp(res[n].url, "http://", 7) == 0 || strncmp(res[n].url, "https://", 8) == 0)) {
            ++n;
        } else if (p) {
            ++p;
        }
...
static float score_rel(const char *query, const RagResult *r) {
    ...
    memcpy(q, query ? query : "", len); q[len] = '\0';
    for (size_t i = 0u; i < len; ++i) q[i] = (char)tolower((unsigned char)q[i]);
...
static void tr_add(RagCtx *ctx, TraceKind kind, float conf, uint32_t t0, const char *fmt, ...) {
    ...
    TraceStep *step = &ctx->trace[ctx->n_steps++];
    step->kind = kind;
    step->elapsed_ms = ms_now() - t0;
    step->confidence = conf;
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(step->detail, sizeof(step->detail), fmt, ap);
    va_end(ap);
}
...
RagCtx *casper_rag_query(const char *q, RagBackend backend, const char *rules_path) {
    ...
    RagCtx *ctx = (RagCtx *)calloc(1u, sizeof(*ctx));
    if (!ctx) return NULL;
    ...
    const char *host = "html.duckduckgo.com";
    if (backend == RAG_BACKEND_BING) host = "www.bing.com";
    const char *path_fmt = backend == RAG_BACKEND_BING ? "/search?q=%s&setlang=en" : "/html/?q=%s";
    char path[640];
    (void)snprintf(path, sizeof(path), path_fmt, enc);
    ...
    if (!resp || resp->len == 0u) {
        tr_add(ctx, TRACE_WARN, 0.0f, t0, "offline");
        hbuf_free(resp);
    ...
    ctx->n_results = parse_ddg(resp->buf, ctx->results, RAG_MAX_RESULTS);
    hbuf_free(resp);
    ...
    for (int i = 0; i < ctx->n_results; ++i) {
        ...
        size_t ul = strlen(ctx->results[i].url);
        size_t sl = strlen(ctx->results[i].snippet);
        size_t n = ul + 1u + sl;
        char *anchor = (char *)malloc(n);
        if (!anchor) {
            memset(ctx->results[i].sha256, 0, sizeof(ctx->results[i].sha256));
            continue;
        }
        memcpy(anchor, ctx->results[i].url, ul);
        anchor[ul] = '\0';
        memcpy(anchor + ul + 1u, ctx->results[i].snippet, sl);
        niyah_sha256((const uint8_t *)anchor, n, ctx->results[i].sha256);
        free(anchor);
    }

    ctx->confidence = ctx->n_results > 0 ? total_score / (float)ctx->n_results : 0.0f;
    tr_add(ctx, TRACE_RANK, ctx->confidence, t0, "results %d", ctx->n_results);

    size_t cp = 0u;
    for (int i = 0; i < ctx->n_results; ++i) {
        int wrote = snprintf(ctx->context + cp, RAG_CONTEXT_MAX - cp,
...
static void json_escape(char *out, size_t max, const char *s) {
    size_t o = 0u;
    if (!out || max == 0u) return;
    if (!s) { out[0] = '\0'; return; }
    for (const unsigned char *p = (const unsigned char *)s; *p && o + 2u < max; ++p) {
        unsigned char c = *p;
        if (c == '"' || c == '\\') {
            out[o++] = '\\'; out[o++] = (char)c;
        } else if (c == '\n') {
...
char *casper_rag_to_json(const RagCtx *ctx) {
    ...
    if (!out) return NULL;
    char query[1024], chain[65];
    json_escape(query, sizeof(query), ctx->query);
    niyah_hash_to_hex(ctx->chain_hash, chain);
    int n = snprintf(out, cap,
        "{\"query\":\"%s\",\"confidence\":%.3f,\"elapsed_ms\":%u,\"chain_hash\":\"%s\",\"n_sources\":%d,\"n_steps\":%d}",
        query, (double)ctx->confidence, ctx->elapsed_ms, chain, ctx->n_results, ctx->n_steps);
