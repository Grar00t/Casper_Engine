...
static HBuf *http_get(const char *host, const char *path) {
    ...
    WinHttpAddRequestHeaders(request, L"User-Agent: Casper/1.0\r\nAccept: text/html,application/xhtml+xml\r\n", (DWORD)-1, WINHTTP_ADDREQ_FLAG_ADD);
...
bool casper_rag_online(RagBackend backend){
    ...
    const wchar_t*host=backend==RAG_BACKEND_BING?L"www.bing.com":L"html.duckduckgo.com";HINTERNET session=WinHttpOpen(L"Casper/1.0",WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,WINHTTP_NO_PROXY_NAME,WINHTTP_NO_PROXY_BYPASS,0);if(!session)return false;HINTERNET connection=WinHttpConnect(session,host,INTERNET_DEFAULT_HTTPS_PORT,0);bool ok=connection!=NULL;if(connection)WinHttpCloseHandle(connection);WinHttpCloseHandle(session);return ok;
#else
    (void)backend;return false;
#endif
}

RagCtx*casper_rag_query(const char*q,RagBackend backend,const char*rules_path){(void)rules_path;if(!q||!q[0])return NULL;RagCtx*ctx=(RagCtx*)calloc(1u,sizeof(*ctx));if(!ctx)return NULL;uint32_t t0=ms_now();strncpy(ctx->query,q,sizeof(ctx->query)-1u);tr_add(ctx,TRACE_PARSE,0.0f,t0,"parsed %zu",strlen(q));
#ifdef _WIN32
    if(backend!=RAG_BACKEND_DDG&&backend!=RAG_BACKEND_BING){tr_add(ctx,TRACE_WARN,0.0f,t0,"unsupported backend");ctx->elapsed_ms=ms_now()-t0;return ctx;}
    char enc[512];url_enc(q,enc,sizeof(enc));const char*host=backend==RAG_BACKEND_BING?"www.bing.com":"html.duckduckgo.com";const char*fmt=backend==RAG_BACKEND_BING?"/search?q=%s&setlang=en":"/html/?q=%s";char path[640];(void)snprintf(path,sizeof(path),fmt,enc);tr_add(ctx,TRACE_SEARCH,0.0f,t0,"GET https://%s%s",host,path);HBuf*resp=http_get(host,path);
#else
    (void)backend;HBuf*resp=NULL;
#endif
    ...
    float total_score=0.0f;for(int i=0;i<ctx->n_results;++i){ctx->results[i].score=score_rel(q,&ctx->results[i]);total_score+=ctx->results[i].score;size_t ul=strlen(ctx->results[i].url),sl=strlen(ctx->results[i].snippet),n=ul+1u+sl;uint8_t*anchor=(uint8_t*)malloc(n?n:1u);if(!anchor){memset(ctx->results[i].sha256,0,sizeof(ctx->results[i].sha256));continue;}memcpy(anchor,ctx->results[i].url,ul);anchor[ul]=0;memcpy(anchor+ul+1u,ctx->results[i].snippet,sl);niyah_sha256(anchor,n,ctx->results[i].sha256);free(anchor);}qsort(ctx->results,(size_t)ctx->n_results,sizeof(ctx->results[0]),result_cmp);ctx->confidence=ctx->n_results?total_score/(float)ctx->n_results:0.0f;tr_add(ctx,TRACE_RANK,ctx->confidence,t0,"results %d",ctx->n_results);
    size_t cp=0u;for(int i=0;i<ctx->n_results;++i){if(cp>=RAG_CONTEXT_MAX-1u)break;int wrote=snprintf(ctx->context+cp,RAG_CONTEXT_MAX-cp,"[%d] %s\n%s\n\n",i+1,ctx->results[i].title,ctx->results[i].snippet);if(wrote<0)break;size_t add=(size_t)wrote;if(add>=RAG_CONTEXT_MAX-cp){cp=RAG_CONTEXT_MAX-1u;break;}cp+=add;}tr_add(ctx,TRACE_CONTEXT,ctx->confidence,t0,"context %zu",cp);niyah_sha256((const uint8_t*)ctx->context,cp,ctx->chain_hash);tr_add(ctx,TRACE_COMPOSE,ctx->confidence,t0,"conf=%.3f",(double)ctx->confidence);ctx->elapsed_ms=ms_now()-t0;return ctx;}
